    ...
    client.manager
      ...
      .on("playerCreate", async (player) => {
          ...
          var embed = new MessageEmbed();
            embed.setTitle(`:thumbsup: **Bergabung ke** \`${client.channels.cache.get(player.voiceChannel).name}\``)
            embed.setDescription(`**Perintah dibuat di: ** <#${client.channels.cache.get(player.textChannel).id}>`)
      ...
      .on("playerMove", async (player, oldChannel, newChannel) => {
        if (!newChannel) {
            ...
            embed.setTitle(`${emoji.msg.ERROR} Antrian telah berakhir.`)
            embed.setDescription(`Meninggalkan channel: \`🔈 ${client.channels.cache.get(player.voiceChannel).name}\``)
            var irc = await isrequestchannel(client, player.textChannel, player.guild);
          ...
          try {
            client.channels.cache.get(player.textChannel).messages.fetch(player.get("playermessage")).then(msg => {
                msg.delete({
                  timeout: 1500
                }).catch(e => console.log("Maaf, tidak dapat menghapus pesan yang anda perintahkan".grey));
            });
          } catch (e) {
      ...
      .on("trackStart", async (player, track) => {
        try {
          ...
          var embed = new MessageEmbed().setColor(ee.color)
            ...
            embed.addField(`**${emoji.msg.time} Durasi: **`, `\`❯ ${track.isStream ? `LIVE STREAM` : format(track.duration)}\``, true)
            embed.addField(`**${emoji.msg.song_by} Pemilik Lagu:**`, `\`❯ ${track.author}\``, true)
            embed.addField(`**${emoji.msg.repeat_mode} Antrian Musik:**`, `\`❯ ${player.queue.length} Songs\``, true)
            embed.setFooter(`Diminta oleh: ${track.requester.tag}`, track.requester.displayAvatarURL({dynamic: true}));
          var irc = await isrequestchannel(client, player.textChannel, player.guild);
          if(irc) {
